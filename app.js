...
app.get('/', function(request, response) {
  fs.readFile('./static/Chess.html', function(err, data) {
    if(err) {
      response.send('에러')
    } else {
...
io.sockets.on('connection', function(socket) {
  ...
  socket.on('newUser', function(name) {
    ...
    io.sockets.emit('update', {type: 'connect', name: 'SERVER', message: name + '님이 접속하였습니다.', people: 접속인원, 참가인원:참가인원()}) 
  })
  /* 전송한 메시지 받기 */
  ...
  socket.on('disconnect', function() {
    ...
    if(socket.id===p1아이디||socket.id===p2아이디){
      if(게임중===1){
        ...
        io.sockets.emit('탈주', {type: '탈주',탈주자아이디:socket.id})  
      }
    }
    if(socket.id===p1아이디&&게임중===0){
      ...
      p1=''
      p1아이디=''
      io.sockets.emit('checkgame', {type: 'checkgame', p1:p1, p2:p2, p1아이디:p1아이디, p2아이디:p2아이디, 참가인원:참가인원()})
    }
    if(접속인원===0){
      게임중=0
    }
    ...
    socket.broadcast.emit('update', {type: 'disconnect', name: 'SERVER', message: socket.name + '님이 나가셨습니다.', people: 접속인원, 참가인원:참가인원()});
  })  
  ...
  socket.on('isitstart', function(data) {
    io.to(data.내소켓아이디).emit('isitstart',{type: 'isitstart', 게임중:게임중,p1:p1})
  })


  
  socket.on('readybutton', function(data) { //누군가 참가를 눌렀을 때
    if(참가인원()===0){ //두칸 다 비어있으면
      p1=socket.name
      p1아이디=socket.id
    }else if(참가인원()===1){ //한칸만 비어있으면
      p2=socket.name
      p2아이디=socket.id
    }
    게임시작여부결정(data)
  })
  socket.on('quitbutton', function(data) { //누군가 취소를 눌렀을 때
    p1=''
    p1아이디=''
    게임시작여부결정(data)
  })
  ...
  socket.on('turnend', function(data) {
    ...
    io.to(data.상대소켓아이디).emit('turnend',{type:'turnend',일차기보:data.일차기보,오십수체크:오십수체크1})
  })
  socket.on('movefinish', function(data) {
    ...
    socket.broadcast.emit('movefinish', {type: 'movefinish',nx: data.nx,ny: data.ny,tx:data.tx,ty:data.ty,특수이동:data.특수이동,내가방금y열폰을두칸움직임:data.내가방금y열폰을두칸움직임, 상대색깔:data.상대색깔});  
  })
  socket.on('kill', function(data) {
  console.log(data.잡은말종류,'가 잡혔습니다.')
  io.sockets.emit('kill', {type: 'kill',잡은말종류: data.잡은말종류,잡은말색깔:data.잡은말색깔});  
  })
  ...
  socket.on('gameend', function(data) {
    ...
    p1아이디=''
    p2아이디=''
    io.sockets.emit('gameend', {type: 'gameend',승자:data.승자,정산결과:data.정산결과})   
  })
  ...
  io.sockets.emit('checkgame', {type: 'checkgame', p1:p1, p2:p2, p1아이디:p1아이디, p2아이디:p2아이디, 선턴정하기신호: a, 참가인원:참가인원()})
  }  
